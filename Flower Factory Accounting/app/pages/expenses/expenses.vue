  ...
  <view class="page">
    ...
    <scroll-view scroll-y class="list-scroll">
      <view class="card" v-for="item in list" :key="item._id" @longpress="deleteItem(item)">
        <view class="card-body">
          ...
          <text class="card-amount red">-{{ (item.amount || 0).toFixed(2) }}元</text>
        </view>
    ...
    <view class="overlay" v-if="showDialog" @tap="closeDialog">
      <view class="dialog" @tap.stop>
        <text class="dialog-title">添加支出</text>
        <input class="dialog-input" placeholder="类别（如：材料、运费）" v-model="formCategory" />
...
function load() { api.getExpenses(monthStr()).then(r => { list.value = r.data }) }
...
function showAdd() {
  const d = new Date(); formCategory.value = ''; formAmount.value = ''
...
function save() {
  if (!formCategory.value.trim() || !formAmount.value) { uni.showToast({ title: '请填写完整', icon: 'none' }); return }
  uni.showLoading({ title: '保存中...' })
  api.addExpense({ category: formCategory.value.trim(), amount: parseFloat(formAmount.value), date: formDate.value, note: formNote.value })
...
function deleteItem(item) {
  uni.showModal({ title: '删除', content: '确定删除这笔支出？',
    success: (res) => { if (res.confirm) api.deleteExpense(item._id).then(() => { uni.showToast({ title: '已删除' }); load() }) } })
}
</script>
