<template>
  <view class="page">
    ...
    <scroll-view scroll-y class="list-scroll">
      <view class="card" v-for="o in orders" :key="o._id" @tap="goDetail(o._id)" @longpress="deleteOrder(o)">
...
function load() { api.getOrders(tab.value).then(r => { orders.value = r.data }) }
...
function deleteOrder(o) {
  uni.showModal({ title: '删除订单', content: '确定删除此订单？',
    success: (res) => { if (res.confirm) api.deleteOrder(o._id).then(() => { uni.showToast({ title: '已删除' }); load() }) } })
}
</script>
